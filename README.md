...
`MASTRA_HOST` and `PORT` are declared in `.env.schema`, which is the list of keys — but `npm run dev`, the path this procedure uses, is bare `mastra factory dev` and never opens that file, so both reach the server with no schema involvement at all, and declaring them gives them no default. The bullets above are the values for this first bring-up, which stays on loopback; `ops/README.md` is the record for what those two and `MASTRACODE_PUBLIC_URL` must contain once the deployment is public.
